...
struct update_position{
    ...
    double operator()(const tuple_double3 &r){
        double _r = thrust::get<0>(r);
        double _v = thrust::get<1>(r);
        double _f = thrust::get<2>(r);
        return _r + _v * dt + 0.5 * _f * dt * dt;
    }
};

struct update_velocity_BD{
    double dt;
    double friction;
    double strength;

    update_velocity_BD(double _dt,double _friction,double _strength){
        dt = _dt;
        friction = _friction;
        strength = _strength;
    }
...
struct Atoms{
    ...
    void updateVeloBD(curandGenerator_t gen,double dt){
        double noise_strength = sqrt(2.0 * friction * temperature * dt);
        create_thermal_noise(gen,1.0);
        auto velo_update = update_velocity_BD(dt,friction,noise_strength);
        thrust::transform(
    ...
    void updatePosMD(double dt){
        //TO DO : using async transform makes this function much faster probably
        auto update_pos = update_position(dt);

        thrust::transform(
            thrust::make_zip_iterator(thrust::make_tuple(x,vx,fx)),
            thrust::make_zip_iterator(thrust::make_tuple(x + N,vx + N ,fx + N)),
            x,
            update_pos
        );
        thrust::transform(
            thrust::make_zip_iterator(thrust::make_tuple(y,vy,fy)),
            thrust::make_zip_iterator(thrust::make_tuple(y + N,vy + N ,fy + N)),
            y,
            update_pos
        );
    }

    void updateVeloMD(double dt){
        auto velo_update = [=]__device__(double _v,double _fx){return _v + 0.5 * _fx * dt;};
        thrust::transform(vx,vx+N,fx,vx,velo_update);
        thrust::transform(vy,vy+N,fy,vy,velo_update);
    }

    void updatePosQuench(double dt){
...
double2 calcForceSoft(double2 pos1, double2 pos2, double size,double L) {
	...
	if(r < cutoff) {
        double invr = 1.0 /r;
        double s_invr = size * invr;
        s_invr *= s_invr;
		double invR = s_invr * s_invr * s_invr;
		force = 12.0 * invR * invR * invr * invr * dr;
	}
	return force;
}
